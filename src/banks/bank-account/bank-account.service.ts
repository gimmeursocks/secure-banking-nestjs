...
export class BankAccountService{
        ...
        try {
            ...
            for (const key in account) {
              if (Object.prototype.hasOwnProperty.call(account, key)) {
                  encryptedUserData[key] = this.encryptionService.encryptData(account[key]);
                }
                console.log(key);
    ...
    async findByNum(num: string): Promise<BankAccount | null>{
        try{
            ...
            const existingAccount = await BankAccount.findOne({ where: {  account_num: encryptedNum } })
                if (existingAccount) {
                    for (const key in existingAccount.dataValues) {
                    if (existingAccount.dataValues[key] && key != "updatedAt" && key != "createdAt") {
